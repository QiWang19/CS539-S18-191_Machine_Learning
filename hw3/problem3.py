...
class Bag(DT):
    ...
    def inference(T,x):
        ...
        y_value_l = list(set([listy[i] for i in range(len(listy))]))
        vote_obj = y_value_l[0]
        num = 0
        for yValue in y_value_l:
            vote_temp = listy.count(yValue)
            if vote_temp >= num:
                num = vote_temp
                vote_obj = yValue
        y = vote_obj
    ...
    def predict(T,X):
        ...
        for i in range(n):
            Y.append(Bag.inference(T, X[:, i].T.flatten()))
        Y = np.array(Y)
