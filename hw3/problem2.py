...
class DT(Tree):
    ...
    def best_threshold(X,Y):
        ...
        for index_cp in cp:
            ...
            temp1 = Tree.entropy(y_1)
            temp2 = Tree.entropy(y_2)
            cent = (float(y_1.shape[0]) / Y.shape[0]) * temp1 + (float(y_2.shape[0]) / Y.shape[
                0]) * temp2
            g_new = ent - cent
    ...
    def best_attribute(self,X,Y):
        ...
        p, n = X.shape
        th, max_g = self.best_threshold(X[0], Y)
        i = 0

        for j in range(1, p):
            tmp_th, tmp_g = self.best_threshold(X[j], Y)
    ...
    def split(X,Y,i,th):
        ...
        C1 = Node(np.asarray(np.empty(shape=[X.shape[0], 0])), list());
        C2 = Node(np.asarray(np.empty(shape=[X.shape[0], 0])), list());

        for idx in range(X.shape[1]):
            if (X[i, idx] <= th):
                C1.X = np.column_stack((C1.X, X[:, idx]))
                C1.Y = np.append(C1.Y, Y[idx])
            else:
                C2.X = np.column_stack((C2.X, X[:, idx]))
                C2.Y = np.append(C2.Y, Y[idx])
    ...
    def build_tree(self, t):
        ...
        if (Tree.stop1(t.Y) or Tree.stop2(t.X)) == True:
            t.isleaf = True
        else:
    ...
    def predict(t,X):
        ...
        for i in range(n):
            y.append(DT.inference(t, X[:, i].T.flatten()))
        y = np.array(y)
    ...
    def load_dataset(filename='data2.csv'):
        ...
        dataset = np.genfromtxt('data2.csv', delimiter=',', dtype=None)
        X = dataset[1:, 1:].T
        X = X.astype(float)
        Y = dataset[1:, 0]
