...
class Tree(object):
    ...
    def entropy(Y):
        ...
        y = dict()
        for element in Y:
            y[element] = y.get(element, 0) + 1

        e = 0
        for name in y:
            prob = y[name] / float(len(Y))
            e += - prob * np.log2(prob)
    ...
    def conditional_entropy(Y,X):
        ...
        for key, value in x.items():
            p = float(value) / len(X)
            y_x = Y[np.argwhere(X == key).flatten()]
            ce += p * Tree.entropy(y_x)
    ...
    def split(X,Y,i):
        ...
        for key in attribute:
            indices = np.argwhere(X[i] == key).flatten()
            C[key] = Node(X[:, indices], Y[indices])
    ...
    def stop1(Y):
        ...
        #########################################
        ## INSERT YOUR CODE HERE
        ylist = set(Y[i] for i in range(Y.shape[0]))
        if len(ylist) > 1:
            s = False
        else:
            s = True
    ...
    def load_dataset(filename='data1.csv'):
        ...
        dataset = np.genfromtxt('data1.csv', delimiter=',', dtype=None)
        X = dataset[1:, 1:].T
        Y = dataset[1:, 0]
