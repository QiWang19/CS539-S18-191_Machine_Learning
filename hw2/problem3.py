...
def compute_a(z):
    ...
    #a = np.exp(z - np.max(z)) / np.sum(np.exp(z - np.max
    BOUND = 1000
    c = z.shape[0]
    zx = np.mat(np.zeros((c, 1)))
    z_max = np.max(z)
    for i in range(0,c):
        zx[i] = (z[i] - z_max)
        #zx[i] = z[i] - z_max
...
def compute_dL_da(a, y):
    ...
    dL_da[y, 0] = -1 / a[y, 0] if a[y, 0] != 0 else -1e6
...
def compute_dL_dW(dL_dz,dz_dW):
    ...
    ## INSERT YOUR CODE HERE
    c = dz_dW.shape[1]
    dL_dW = np.asmatrix(np.zeros((dz_dW.shape[0], dz_dW.shape[1])))
    for i in range(c):
        dL_dW[:, i] = np.multiply(dz_dW[:, i], dL_dz)
